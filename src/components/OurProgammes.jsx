  ...
  return (
    <div className='py-[60px] bebas-neue-regular w-full px-4 md:px-6'>
      <div className='max-w-6xl mx-auto px-4 md:px-6'>
        <h2 className='text-xl md:text-3xl lg:text-[42px] font-bold text-center mb-10 md:mb-20'>OUR PROGRAMMES</h2>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-x-10 md:gap-x-24 gap-y-8 md:gap-y-14 w-[95%] mx-auto'>
          {programmes.map((programme, index) => (
            <div key={index} className='flex items-start space-x-4 mb-4'>
              <img src={programme.image} alt={`${programme.title} Icon`} className='w-16 md:w-20 lg:w-24 mr-2 md:mr-3' />
              <div className='flex flex-col gap-y-1 md:gap-y-2'>
                ...
                <p className='text-gray-700 text-sm md:text-base tracking-wide raleway-400'>{programme.description}</p>
              </div>
            </div>
          ))}
        </div>
